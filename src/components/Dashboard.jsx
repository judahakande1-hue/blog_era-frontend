...
function Dashboard() {
  ...
  return (
    <section className="min-h-screen mb-5 w-full p-8">
      ...
      <div className="mt-10 bg-gray-100 h-fit p-5 rounded-2xl">
        <div className="grid grid-cols-1 mb-5 gap-5 sm:grid-cols-2 lg:grid-cols-5">
          <div className="bg-white rounded-xl p-5 w-full text-center">
        ...
        <div className="mt-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <h1 className="font-semibold font-[JetBrains Mono]">Writer Tools</h1>

          <div className="flex flex-col sm:flex-row gap-3">
            <Link
              to="/dashboard/create"
              className="text-center hover:bg-purple-50 text-purple-600 px-4 py-2 rounded-md border border-purple-600"
            >
              Create New Post
            </Link>
            ...
            <Link
              to="/dashboard/MyPost"
              className="text-center bg-purple-500 hover:bg-purple-600 text-white px-4 py-2 rounded-md"
            >
              My Posts
            </Link>
          </div>
        ...
        <div className="mt-12 bg-white p-5 rounded-xl">
          <h1 className="font-semibold font-[JetBrains Mono] mb-5">
            My Recent Posts
          </h1>
            ...
            <div className="space-y-5">
              {recentPosts.map((post) => (
                <div key={post._id} className="border-b border-gray-200 pb-4">
                  <h2 className="font-bold mb-2 text-lg">{post.title}</h2>
                  <p className="text-purple-600">{post.category}</p>
                  <p className="text-gray-600">
                    {post.content.slice(0, 120)}...
                  </p>
                  <p className="text-gray-700 py-1">Status: {post.status}</p>
                  <Link
                    to={`/dashboard/view/${post._id}`}
                    className="text-purple-600 py-2 rounded-md"
                  >
                    View full post
                  </Link>
                </div>
